...
export const OverviewLocationPage = () => {
  ...
  const [locations, setLocations] = useState<BadmintonVenue[]>([]);
  const [curentLocation, setCurentLocation] = useState<BadmintonVenue>();
  const [reload, setReload] = useState(0);
  const user = useSelector((state: RootState) => state.auth.login.currentUser);
  const columns: TableProps<BadmintonVenue>["columns"] = [
    {
      title: "Tên",
      dataIndex: "name",
      key: "name",

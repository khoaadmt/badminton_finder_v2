  ...
  useEffect(() => {
    ...
    bookingService
      ...
      .then((response) => {
        if (response.data) {
          const totalSales = response.data.reduce((acc: number, trans: any) => {
            console.log("trans.shift.price :", trans.shift.price);
            return acc + trans.shift.price;
          }, 0);
        ...
        <ColCard
          ...
          body={
            <Trend
              ...
              totalSalesYesterday={
                totalSalesYesterday ? totalSalesYesterday : 1
              }
            />
          }
          footer={
            <Field
              name={"Doanh số hôm qua"}
              number={numeral(totalSalesYesterday).format("0.0a")}
            />
          }
        />
        {/* <ColCard
